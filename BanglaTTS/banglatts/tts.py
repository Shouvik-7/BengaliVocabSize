...
def getsample(filename, bangla_tts):
    avg = []
    for n in range(10):
        df = pd.read_csv(f'./Samples/{n}.csv')
        know = []

        for i in range(len(df)):
            word = df['words'].iloc[i]
            print(word)
            bangla_tts(word, f'test{n}_{i}.wav', voice='female')
            winsound.PlaySound(f'test{n}_{i}.wav',winsound.SND_FILENAME)
            know.append(int(input('enter 1 if word is known else 0:')))
        df['know'] = know
        print(df)
        avg.append(sum(know)/20)
    print(avg)
    print(f'score: {(sum(avg) / 10)}')
